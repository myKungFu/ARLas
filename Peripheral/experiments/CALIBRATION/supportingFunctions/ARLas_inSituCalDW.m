function [isc] = ARLas_inSituCalDW(obj,recording,stimulus,recordingRef)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [isc] = ARLas_inSituCalDW(obj,recording,stimulus);
%
...
%
% This version (DW) is for the Lichtenhan lab. It assumes that an ear bar
% is being used (for guinea pigs). Also contains reference inputs for
% comparison (optional)
%
...
if ~isempty(recordingRef)
    Rref = fft(recordingRef,nfft); % recordings vector
    PLref = Rref ./ S; % load pressure
    PLref = PLref(1:obj.fmaxIndx); % cut to highest calibrated frequency
    PLref = PLref(obj.fminIndx:end);
end
...
% force a correction here...
% cavityDiameter = 0.3; % cm This is the known cavity diameter
% cavityTemperature = 28; % degrees C
% d = cavityTemperature - 26.85; % from Keefe (1984)
% cc = 3.4723e4 * (1 + 0.00166 * d); % speed of sound, adjusted for temperature, from Keefe (1984)
% rho = 1.1769e-3 * (1 - 0.00335 * d);
% r = cavityDiameter / 2; % radius in cm
% z0 = (rho * cc) / (pi * r^2); % z0 = Ro; characteristic impedance (z0) is constant & real; 
% diam = 0.3;
...
if ~isempty(recordingRef)
    splref = 20*log10(Amax*abs(PLref)/pRef);  % load sound pressure magnitude (dB spl)
else
    splref = [];
end
...
isc.splref = splref; % maximum sound pressure level magnitude (dB re: 20 uPa) from reference mic
