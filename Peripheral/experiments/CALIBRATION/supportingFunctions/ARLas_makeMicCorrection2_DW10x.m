function [b] = ARLas_makeMicCorrection2_DW10x(fplWaveform1,refWaveform1,fplWaveform2,refWaveform2,fmin,fmax,fs)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% b = ARLas_makeMicCorrection2_DW10x(fplWaveform1,refWaveform1,fplWaveform2,refWaveform2,fmin,fmax,fs);
%
% Create microphone correction for the ER10X probe system. 
% This versions averages the two responses made using each channel
...
%
% Input Arguments:
%  fplWaveform1 = calculated forward pressure level in the calibration assembly.
%  refWaveform1 = reference waveform measured by the condensor mic.
...
% --------------------- Adjustable Parameters -----------------------------
    doSmoothing = 0;
    doPhaseAdjust = 1;
    filterGroupDelay = 0.001; % desired filter group delay (s)
                              % for DW, 1 ms group delay puts you within 1 dB of target values
    smoothing = 0.000000001;  % spling smoothing value
%--------------------------------------------------------------------------    
    
    N = length(fplWaveform1); % length if input waveform
    N2 = length(refWaveform1);
    ...
    if mod(N,2)~=0 % ensure an even number of samples
        fplWaveform = [fplWaveform1;0];
        refWaveform = [refWaveform1;0];
        fplWaveform = [fplWaveform2;0];
        refWaveform = [refWaveform2;0];
        N = length(fplWaveform1); 
    end
    
    FPL1 = fft(fplWaveform1,N); % put into the frequency domain
    REF1 = fft(refWaveform1,N);
    FPL2 = fft(fplWaveform2,N); % put into the frequency domain
    REF2 = fft(refWaveform2,N);
    
    FPLm = (abs(FPL1) + abs(FPL2))./ 2;
    FPLp = (angle(FPL1) + angle(FPL2))./ 2;
    FPL = FPLm .* cos(FPLp) + 1i*FPLm .* sin(FPLp);
    REFm = (abs(REF1) + abs(REF2))./ 2;
    REFp = (angle(REF1) + angle(REF2))./ 2;
    REF = REFm .* cos(REFp) + 1i*REFm .* sin(REFp);
