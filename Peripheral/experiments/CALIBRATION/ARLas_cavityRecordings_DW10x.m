function [] = ARLas_cavityRecordings_DW10x(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ARLas_cavityRecordings_DW10x(varargin);
%
% Make recordings from ER10x calibrator for calculation of Thevenin source
% parameters. This version (DW10X) is optimized for calibrations in the
% Lichtenhan lab. Assumed that you are using a set of custom stainless
% tubes 0.3 cm in diameter, of the lengths [2.9; 3.6; 4.15; 5.2; 6.9] cm
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    adjust = -0.6356; % (cm); reduction due to insertion of the probe tip
    cavityLengths = cavityLengths + adjust;
    cavityDiameter = 0.3; % small stainless cavity diameter (cm)
    %stimGain_dB = -15; % reduce so you don't overdrive the system; 
% changed this 2/10/2023 -- ssg -- USF
    stimGain_dB = -30; % reduce so you don't overdrive the system; 
    ...
    for jj=1:nCavities % loop over number of cavities
        ...
        for kk=1:nChannelsOut % loop over output channels; test output channels one at a time
            ...
            if kk == 1
                [thevRecording1(:,jj),stimf] = cleanDataDW(data,obj.fs,stimulus,header,cut1,cut2);
            elseif kk == 2
                [thevRecording2(:,jj),stimf] = cleanDataDW(data,obj.fs,stimulus,header,cut1,cut2);
            end
        end
    end
    ...
    t = thevCal_EB(dummy.recordingParams,dummy.stimulus,dummy.recordings);
    t.fmin = fmin;
    t.fmax = fmax;
    t.calculate
...
function [stim,F,cut1,cut2] = getStimulus(fs)
    ...
    fmax = 42000; % ending frequency (Hz); this is 1.2 times the desired ending frequency. 
