function [SFOAE,SFOAE_ref] = sfoaeAnalysis_v5(header,Data,SFOAE,RefData)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [DPOAE,DPOAE_ref] = sfoaeAnalysis_v5(header,Data,SFOAE,RefData);
%
% Analyze swept SFOAE recordings; goes with sfoae_v5.m.
%
% Author: Shawn S. Goodman
% Date: February 2, 2022
% Last Updated: February 1, 2022 -- ssg -- based on dpoaeAnalysis_v5.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    %tic % each analysis takes about 0.2 minutes, or 13 sec. (on a super
    %fast computer!)
...
%     end
    ...
    [P1,P2,P12,PD,Y1c,Y1,Y2c,Y2,Ydpc,Ydp,F2,F1,Time] = clean(Data,nSweeps,Y1c,Y1,Y2c,Y2,Ydpc,Ydp,F2,F1,Fdp,Time);
    X = PD;
    ...
    stepSize = 8; % 12 max, 6 min in a good range
    jj = 1;
...
% ->> process here
    ...
    if ~isempty(RefData)
        ...
        SFOAE_ref.N2_r = N2_r;
        SFOAE_ref.P2_r = P2_r;
        SFOAE_ref.Ldp_r = Ldp_r;
        SFOAE_ref.Ndp_r = Ndp_r;
        SFOAE_ref.Pdp_r = Pdp_r;
        SFOAE_ref.f1 = f1;
        SFOAE_ref.f2 = f2;
        SFOAE_ref.fdp = fdp;
    else
...
% put into octave averages
    ...
    counter = size(SFOAE.L1,2);
    counter = counter + 1;
    SFOAE.targetL1(:,counter) = header.userInfo.targetL1;
    SFOAE.targetL2(:,counter) = header.userInfo.targetL2;
    SFOAE.L1(:,counter) = L1;
    SFOAE.N1(:,counter) = N1;
    SFOAE.P1(:,counter) = P1;
    SFOAE.L2(:,counter) = L2;
    SFOAE.N2(:,counter) = N2;
    SFOAE.P2(:,counter) = P2;
    SFOAE.Ldp(:,counter) = Ldp;
    SFOAE.Ndp(:,counter) = Ndp;
    SFOAE.Pdp(:,counter) = Pdp;
    SFOAE.f1(:,counter) = f1;
    SFOAE.f2(:,counter) = f2;
    SFOAE.fdp(:,counter) = fdp;
    SFOAE.Ldp_3oct(:,counter) = Ldp_3oct;
    SFOAE.Ndp_3oct(:,counter) = Ndp_3oct;
    SFOAE.fc(:,counter) = fc;
...
function [L1,N1,P1,L2,N2,P2,Ldp,Ndp,Pdp,f1,f2,fdp] = doAnalysis(nIterations,X,F1,F2,start,finish,Y1c,Y1,Y2c,Y2,Fdp,fs,wn,stepSize)
    ...
    for jj=1:nIterations
        ...
        for ii=1:newCols
            ...
            %w = meanSmoother(w,n);
            ww(ii,1) = sum(w);
            [B1(ii,1),B2(ii,1),Bdp(ii,1)] = dpoae_OLSfit(y1c,y1s,y2c,y2s,ydpc,ydps,Signal(:,ii),h.*w);
        end
...
function [P1,P2,P12,PD,Y1c,Y1,Y2c,Y2,Ydpc,Ydp,F2,F1,Time] = clean(Data,nSweeps,Y1c,Y1,Y2c,Y2,Ydpc,Ydp,F2,F1,Fdp,Time)
    Rows = size(Y1(:),1);
    Cols = nSweeps*3;
    X = reshape(Data,Rows*3,Cols/3);
