...
%--------------------------------------------------------------------------
    ...
    f2 = [2000,4000, 8000]; % f2 frequency (Hz) 
    nSweeps = 24; % number of test sweeps 24
...
adjust = 0; % hard coded 4/21/2025 ssg
    ...
    if ~isempty(probeInputA)
        ...
        DPOAE_A.fdp = fdp;
        DPOAE_A.doEPL = doEPL;
        [DPOAE_A] = analyzeData(DataA,headerA,LC,DPOAE_A,obj);
    else
        DPOAE_A = [];
    end
    catch ME
    ...
    if ~isempty(probeInputB)
        ...
        DPOAE_B.fdp = fdp;
        DPOAE_B.doEPL = doEPL;
        [DPOAE_B] = analyzeData(DataB,headerB,LC,DPOAE_B,obj);
    else
        DPOAE_B = [];
    end
    catch ME
...
function [DPOAE] = analyzeData(Data,header,LC,DPOAE,obj)
    ...
    nFreqs = length(DPOAE.f2);
    for ii=1:nFreqs
        [signal(:,ii),nf(:,ii),snr(:,ii),targets(:,ii),zbar(:,ii)] = sweptLSF(x,Data,DPOAE.fdp(ii),Lmin,Lmax,step,fs);
    end
    ...
    for ii=1:nFreqs
        ...
        [jar,h1,h2] = ricianLGF_fit2d(signal(1:indxCut,ii),nf(1:indxCut,ii),targets(1:indxCut,ii),nSweeps);
        
        saveAnalysisPlots(obj,LC,h1,h2)

        DPOAE.(['f',num2str(DPOAE.f2(ii))]).jar = jar;
        DPOAE.(['f',num2str(DPOAE.f2(ii))]).L2max = L2max;
        DPOAE.(['f',num2str(DPOAE.f2(ii))]).L2cut = L2cut;
        DPOAE.(['f',num2str(DPOAE.f2(ii))]).indxCut = indxCut;
    end
