...
methods
    ...
    function initGui(varargin) % initialize the arlas graphical user interface
       ...
       obj.H.Position = [887.0000   67.4000  544.8000  744.4000];
            ...
            obj.STEP = uicontrol('Parent',obj.CONTROL,'Style','pushbutton', ...
                'String','5','FontSize',12,'BackgroundColor','white', ...
                'HorizontalAlignment','Left','Units','Normalized', ...
                'Position',[.72 .75 .16 .1], ...
                'Callback',@obj.toggleStepSize);
    ...
    function stepManager(obj, stepVal) % control level step size
    if nargin < 2
        stepVal = str2double(obj.STEP.String); % fallback if called without value
    end
    obj.stepNow = stepVal;
        obj.levels = (obj.minOutput(obj.fIndx):obj.stepNow:obj.maxOutput(obj.fIndx))';
        obj.nLevels = length(obj.levels);
    ...
    function audiometer_plotAG(varargin) % plot audiometer Audiogram
       ...
       if obj.earNow == 1
        ...
        FF = [250,500,1000,2000,4000,8000,16000,18000]';
        LL = (-10:10:110)'+conv;
        ff = log2(FF);
        nf = length(ff);
        ymin = min(LL);
