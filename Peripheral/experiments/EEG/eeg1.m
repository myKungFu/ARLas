...
%
% Author: Shawn Goodman, PhD & Ehsan Khalili
% Auditory Research Lab, the University of Iowa
% Date: August 28, 2023
% Last Updated: August 28, 2023 -- ssg
% Last Updated: September 8, 2023 -- ssg, mek -- 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    obj = varargin{1}; % get the arlas object
    V = '08SEP2023'; % this is the current version number of this program

%------ USER MODIFIABLE PARAMETERS ----------------------------------------
%    targetNoise = 0;
    targetClick = 138;
    nSweeps = 1000;
    dBAtten = -5; % dB off of full output for click
    
    polarity = 'alt'; % polarity: use 'alt', 'rare', 'cond'
    n_eegChannels = 2; % how many eeg channels (1 or 2)

    % NOTES: Click stimulus currently being played through left probe Ch1
%--------------------------------------------------------------------------

offset = 15; % samples of delay difference between ER10X and OptiAmp
scalingFactor = 10^(dBAtten/20);

%    multiplierNoise = 0.25; %0.25;
%    target = 85;
%     saveName = 'Shawn.mat';
%     savePathName = 'C:\myWork\ARLas\Data\MEMRdata\';
...
%--------------------------------------------------------------------------
    ...
    % get the eeg settings
    [inputs,outputs] = hardwareSetup;
    probeInputEEG1 = inputs{3};
    probeInputEEG2 = inputs{4};
    ...
    cps = 11.1; % clicks per second
    len = 1/cps; % sweep length
    N = round(fs*len); % number of samples per sweep
    stimulus = zeros(N,1);
    clickDelay = 0.005; % delay the click from the start of the stimulus
    delayIndx = round(clickDelay * fs);
    stimulus(delayIndx) = 1;
    S1L = stimulus;
    S1R = stimulus * 0;
    S2R = stimulus * 0;
    S2L = stimulus * 0;
    ...
    S1L = S1L / (max(abs(S1L)));
    S1L = S1L * scalingFactor;

    % make condensation and rarefaction versions
    %   [Rarefaction, Condensation]
    %   We made a condensation, but we want to present rarefaction first
    S1L = [-S1L,S1L];

    % ensure amplitude does not exceed 1
    if max(abs(S1R(:))) >= 1
        figure(111);
        plt(S1R(:))
        title('S1R')
...
% ----        
    ...
    if ~isempty(probeInputL)
        probeInputL.label = [probeInputL.label,'_eeg'];
        obj.setRecList(probeInputL.ch,probeInputL.label,probeInputL.micSens,probeInputL.gain);    
    end
    if ~isempty(probeInputR)
        probeInputR.label = [probeInputR.label,'_eeg'];
        obj.setRecList(probeInputR.ch,probeInputR.label,probeInputR.micSens,probeInputR.gain);    
    end
    % eeg probe
    obj.setRecList(probeInputEEG1.ch,probeInputEEG1.label,probeInputEEG1.micSens,probeInputEEG1.gain);    
    if n_eegChannels == 2
        obj.setRecList(probeInputEEG2.ch,probeInputEEG2.label,probeInputEEG2.micSens,probeInputEEG2.gain);    
    end


    obj.setNReps(nSweeps); % number of times to play stimulus
    ...
    obj.objPlayrec.userInfo.polarity = polarity;
    obj.objPlayrec.userInfo.offset = offset;
    ...
    [headerV1,DataV1] = obj.retrieveData(['Ch',num2str(probeInputEEG1.ch)]); % retrive recorded dat
    if n_eegChannels == 2
        [headerV2,DataV2] = obj.retrieveData(['Ch',num2str(probeInputEEG2.ch)]); % retrive recorded dat
    end
    ...
    try 
        ...
        saveFileName = [obj.subjectID,'_analyzedEEG.mat'];
        saveFileName = ARLas_saveName(savePath,saveFileName);
        if n_eegChannels == 2
            save([savePath,saveFileName],'headerV1','DataV1','headerV2','DataV2','headerL','DataL','headerR','DataR')
        else
            save([savePath,saveFileName],'headerV1','DataV1','headerL','DataL','headerR','DataR')
        end
    catch
    ...
    % visual analysis
    ref = 1e-6;
    [frequency,signal,noiseFloor] = ARLas_fda(DataV1,fs,ref);
    h1 = figure;
    plt(frequency,signal,'b')
    hold on
    plt(frequency,noiseFloor,'k')
    xlabel('Frequency (Hz)')
    ylabel('Magnitude (dB re: 1 \muV)')
