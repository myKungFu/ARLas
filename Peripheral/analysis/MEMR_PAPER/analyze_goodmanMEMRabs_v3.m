function [MEMR_inc,MEMR_mem,h1,h2] = analyze_goodmanMEMRabs_v3(header,Clicks,headerN,Noise,subjectName,runNumber)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% basePath =  'C:\myWork\ARLas\Data\dummy\dummy_04OCT2024\dummy_04OCT2024_dummy_run5\';
basePath =  'D:\MEMR_DATA\MEM05\MEM05_27OCT2022_MEM_run1\';
cd(basePath)
    ...
    if stabilityCheck ~=5 % this is hack to make this always happen
       ...
       for jj=1:nFreqs
        ...
        for jj=1:nFreqs
            ...
            tc = abs(d-1); % This is total change vector
            td = sqrt(sum((tc.*abs(baseline)).^2) / sum(abs(baseline).^2));
            d1 = sqrt(sum(abs(z_sm-baseline).^2)) / sqrt(sum(abs(baseline).^2));
            d1 = d1 + 1;
            d2 = abs(abs(d)-1)+1; % magnitude only change

            magp = abs(abs(d)-1)/td ; % Proportion Magnitude
            ppd = 1-(abs(abs(d)-1)/td) ; % proportion Phase
