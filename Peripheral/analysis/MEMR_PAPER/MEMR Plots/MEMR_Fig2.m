...
% [] = MEMR_Fig2()
%
% Figure 2 for paper and ARO talk
%   Shows click separation
%
...
subjectName = 'MEM23'; %'MEM10'; % 'MEM12', 'MEM09', 'MEM17'
runNumber = 1;
...
%     Clicks = Clicks * 1.3;
    ...
    XX = mean(Clicks,2);
    XX = reshape(XX,size(XX,1)/160,160);
    tt = reshape(t,size(t,1)/160,160);
    indx1 = 250;
    indx2 = 500;
    XX = XX(indx1:indx2,:);
    tt = tt(indx1:indx2);
    [~,hat] = max(abs(mean(XX,2)));
    tt = tt - tt(hat);
    tt = tt * 1000; % put into ms
    xx = XX(:,1);
    %xx = repmat(xx,1,size(XX,2));
   ...
   subplot(2,1,1)
    ...
    hy1 = ylabel('Pressure (Pa)','FontSize',14);
    hy1.Position = [ -0.3991    0.0000   -1.0000];
    set(gca,'fontsize',11)
    grid on
    line([tm14,tm14],[ymin,ymax],'Color',[0 0 0],'LineStyle','--','LineWidth',0.5)
    %line([0,0],[ymin,ymax],'Color',[0 0 0],'LineStyle','-','LineWidth',0.5)
    line([t14,t14],[ymin,ymax],'Color',[0 0 0],'LineStyle','--','LineWidth',0.5)
    line([t125,t125],[ymin,ymax],'Color',[0 0 0],'LineStyle','--','LineWidth',0.5)
    box on
   ...
   subplot(2,1,2)
    ...
    ylim([-.03,.03])
    xlabel('Time (ms)','FontSize',14)
    hy2 = ylabel('\Delta Pressure','FontSize',14);
    set(gca,'fontsize',11)        
    grid on
    line([tm14,tm14],[ymin,ymax],'Color',[0 0 0],'LineStyle','--','LineWidth',0.5)
    %line([0,0],[ymin,ymax],'Color',[0 0 0],'LineStyle','-','LineWidth',0.5)
  ...
  tbA = annotation('textbox',...
    ...
    % subject 10: biggest response 40 from peak, responses out to 100 from peak
    % subject 11: biggest response 24 from peak, responses out to 125
    % subject 11: biggest response 34 from peak, responses out to 125, earliest response 16
    % subject 15: biggest response 43 from peak, responses out to 125, 
    % subject 16: biggest response 45 from peak, responses out to 125,
    % earliest response 17 from peak, first delay = 13
    
keyboard


% % FT = fft(XX-xx,9600);
% % FT = 20*log10((abs(FT-1)+1));
% % plot(FT)
% % 
% % figure
% % FT = fft(XX-xx,9600);
...
% % plot(q,'r')
    ...
    nfft = fs;
    stepSize = round(n/2);
    nSteps = round((indx2-indx1)/(n/2));
    start = 1;
    finish = n;
    for jj=1:nSteps-2
        dummy = XX(start:finish,:);
        ttt = tt(start:finish);
        Dummy = fft(dummy,nfft);

        figure(jj)
        subplot(1,3,1)
         plot(round(ttt'*fs)+1,dummy)
        subplot(1,3,2)
         plot(20*log10(abs(Dummy)))
         xlim([500 1500])
        subplot(1,3,3)
         for kk=1:size(dummy,2)
            q = dummy(:,1);
            rms(kk,1) = sqrt(mean((q-dummy(:,kk)).^2));
         end
         y = (abs(Dummy(500:1500,:)-1))+1;
         for kk=1:size(y,1)
            y(kk,:) = y(kk,:) ./ y(kk,1);
         end
         y = mean(y,1);
         ...
         plot(y,'r')
%          hold on
%         y = 20*log10(abs(Dummy(2876,:)));
%         y = y - y(1);
%         plot(y,'g')
%         y = 20*log10(abs(Dummy(4000,:)));
%         y = y - y(1);
%         plot(y,'b')
...
keyboard    
    % subj 6: 330 - 360 was biggest (step 6), about 4 round trip delays
    % subj 15: 320-360 was biggest (steps 5-6)

    figure
    plot(round(tt'*fs)+1,XX)
    hold on
    plot([320,360],[0,0],'g-','LineWidth',2)
