...
%
% Author:
% Date:Updated on 11/9/2023; 
%% Now displaying Hysteresis, calcluating thresholds, showing the max activation, calculating growth rate
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
peakN= find(peakX>3.65);
peakX = peakX(peakN(1)); % function returns two outputs (both the same) take the first
peakY = ppval(pp,peakX); % max average activation
delay = peakX-4; % reflex delay in seconds
%dY = ppval(dfdx,x);

Peak = plot(peakX,peakY,'or')
...
T1 = plot(Thd(1),cut,'*r')
T2 = plot(Thd(2),cut,'*r')
legend('F Bins','Mean','Threshold Cut','Peak','T1','T2')
...
[~, split_index] = min(abs(x - peakX));

part1_x = x(1:split_index);
part1_ysm = ysm(1:split_index);

part2_x = x(split_index+1:end);
part2_ysm = ysm(split_index+1:end);


subplot(3, 1, 3);
plot(part1_x, part1_ysm, 'Color', [0 0 1], 'LineWidth', 1);
hold on
plot(((part2_x)-peakX), flip(part2_ysm), 'Color', [1 0 0], 'LineWidth', 1);
hold on
%title('Ascending vs Descending');
s = area(((part2_x)-peakX), flip(part2_ysm),'FaceColor',[1 0.0 0.0],'EdgeColor',[0 0.5 0.1]);
alpha(s,.75)
hold on
s = area(part1_x, part1_ysm,'FaceColor',[0.0 0.0 1],'EdgeColor',[0 0.5 0.1]);
alpha(s,.75)
ax = gca; % current axes
ax.XGrid = 'on';
ax.YGrid = 'on';
...
[~, peak_index] = min(abs(x - peakX));

% Find the index of X1 and X2
index_xa = find(abs(x - Thd(1)) == min(abs(x - Thd(1))));
index_xd = find(abs(x - Thd(2)) == min(abs(x - Thd(2))));

% Calculate the slopes
slope_ascending = polyfit(x(index_xa:peak_index), ysm(index_xa:peak_index), 1);
slope_descending = polyfit(x(peak_index:index_xd), ysm(peak_index:index_xd), 1);

% Display
disp(['Ascending Threshold: ', num2str(Thd(1))]);
disp(['Descending Threshold : ', num2str(Thd(1))]);
disp(['Ascending Growth Rate: ', num2str(slope_ascending(1))]);
disp(['Descending Growth rate: ', num2str(slope_descending(1))]);
%title(['Growth Rate A and D = ',num2str(slope_ascending(1)),' , ',num2str(slope_descending(1))]);
