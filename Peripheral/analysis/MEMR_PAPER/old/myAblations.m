classdef myAblations < handle
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Class definition myAblations
% For use with DWexplore
% Used in finding best beta distribution fit to SFOAE and CAP data
%
...
methods
    function obj = myAblations(time,data,nf,ablationStart,ablationFinish,A,B,p,d2,p2,residualMag,residualPhi)
        
        obj.nIterations = 100; % bootstrap iterations
        
        % assign variables ------------------------------------------------
        obj.tau = time(:);         % experiment time (min: re injection start)
        obj.Y_raw = data(:);       % magnitudes in original units (uV or uPa)
        obj.sigmaY_raw = nf(:);    % noise floor in original units (uV or uPa)
        obj.p = p;                 % emission phase
        obj.Y_raw2 = d2;           % emission magnitudes with residual removed
        obj.p2 = p2;               % emission phase with residual removed
        obj.residualMag = residualMag; % magnitude of residual
        obj.residualPhi = residualPhi; % phase of residual
        ...
        obj.NN = 500; % create vector xx for densly-spaced distributions the bootstrap
        spacing = (obj.tau(end)-obj.tau(1)) / (obj.NN-1);
        obj.tauTau = ((0:1:obj.NN-1)' * spacing) + obj.tau(1);
        obj.xx = (obj.tauTau - obj.offset) / obj.slope;
        % make x-axis in terms of cochlear distance -----
        % this is Eq. 15 in the paper
        obj.flowRate = 0.5; % flow rate in mm/m
        obj.Ltotal = 20.8; % length of scala media
        obj.d = -obj.flowRate * obj.tau + obj.Ltotal; % x-axis: distance in mm along scala media
        obj.dd = -obj.flowRate * obj.tauTau + obj.Ltotal; % x-axis: dense distance in mm along scala media
        obj.predCf = 0.174*exp(0.1383*obj.tau); % predicted CF, given time delay tau
        obj.predCfCf = 0.174*exp(0.1383*obj.tauTau);
