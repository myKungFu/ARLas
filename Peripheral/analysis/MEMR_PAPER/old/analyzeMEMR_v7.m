function [MEMR] = analyzeMEMR_v7(pathName,fileNameL,fileNameR,subjectName,runNumber)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% --> what are these numbers???    
    ...
    MEMR = runme(Clicks,nSamples,nClicks,indx,time,fs,peakFreqs,noiseLvl,noiseRate,...
        subjectName,runNumber,elicitor,fileNameL,fileNameR,pathName,d.name);
    %keyboard
end
% INTERNAL FUNCTIONS ------------------------------------------------------
function [MEMR] = runme(Clicks,nSamples,nClicks,indx,time,fs,peakFreqs,noiseLvl,noiseRate,...
    subjectName,runName,elicitor,fileNameL,fileNameR,pathName,dname)
...
% keyboard
 ...
 %Chunk = abs(hilbert(Chunk));
 %Chunk(130:end,:) = 0;
        FT = fft(Chunk); % contains all sweeps at all frequencies for a given click position
        fIndx1 = 1;
    ...
    for jj=1:nFreqs
        ...
        mi = imag(m(:));
        t = (1:1:length(mr))';
        smoothing = 0.1; %0.0000000009; % smoothing factor
