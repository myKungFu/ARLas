function [MEMR] = analyzeMEMR_v11(pathName,fileNameL,fileNameR,subjectName,runNumber)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [MEMR] = runme(Clicks,nSamples,nClicks,indx,time,fs,peakFreqs,noiseLvl,noiseRate,...
    ...
    clickN = 194;
    C = zeros(clickN,nSweeps,nClicks);
    ...
    nfft = 960; %length(squeeze(C(:,1,1)));
    frequency = (0:1:nfft-1)'*(fs/nfft);
    fmin = 100;
    fmax = 8000;
    [~,indx1] = min(abs(fmin-frequency));
    [~,indx2] = min(abs(fmax-frequency));
    nFreqs = indx2-indx1+1;
    CFT = zeros(nFreqs,nSweeps,nClicks);
    for ii=1:nSweeps
        cc = squeeze(C(:,ii,:));
        nfft = size(cc,1);
        ...
        %fIndx1 = 1;
        %fIndx2 = size(cc,1)/2;
        FT = FT(indx1:indx2,:); % cut down to size (non-aliased part only)
        CFT(:,ii,:) = complex(FT);
    end
    for ii=1:nFreqs
        cc = squeeze(CFT(ii,:,:));
        ...
        t = (1:1:length(mr))';
        w = ones(size(mr));
        smoothing = 0.00000000001; %0.01; % smoothing factor is a good sm factor
...
% -------------------------------------------------------------------------
    t = (1:1:nClicks)'; % time of click in steps of 1
    ...
    for jj=1:nFreqs
        ...
        m = squeeze(M(jj,:,:)); % size m = 160 x 15
        w = squeeze(W(jj,:,:));
        Mr = real(m);
        Mi = imag(m);
        ww = sum(w,1);
        for kk=1:nIterations
            ...
            mr = sum(Mr(indxkk,:),1)./ ww;
            mi = sum(Mi(indxkk,:),1)./ ww;
