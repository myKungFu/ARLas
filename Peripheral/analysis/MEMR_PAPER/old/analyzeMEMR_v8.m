function [MEMR] = analyzeMEMR_v8(pathName,fileNameL,fileNameR,subjectName,runNumber)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [MEMR] = analyzeMEMR_v8(pathName,fileNameL,fileNameR,subjectName,runName)
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %cd([[pathName,subjectName,d.name]])
...
% --> what are these numbers???    
    ...
    MEMR = runme(Clicks,nSamples,nClicks,indx,time,fs,peakFreqs,noiseLvl,noiseRate,...
        subjectName,runNumber,elicitor,fileNameL,fileNameR,pathName,d.name,doSmoothing);
    %keyboard
        [pl,Pl,phi,other,wf] = ARLas_convertPL(Clicks(:,1),header.userInfo.iscS1L);
...
function [MEMR] = runme(Clicks,nSamples,nClicks,indx,time,fs,peakFreqs,noiseLvl,noiseRate,...
    subjectName,runName,elicitor,fileNameL,fileNameR,pathName,dname,doSmoothing)
...
% Chunk1 = Clicks(indx(1):indx(1)+nSamples,:);
% Chunk80 = Clicks(indx(80):indx(80)+nSamples,:);
% c
% plot(Chunk1,'b')
% hold on
% plot(Chunk80,'r')
% keyboard
        ...
        % NOTE TO SELF: CANNOT***FILER HERE!!!
        %Chunk = ARLas_hpFilter(Chunk,fs,100);
    ...
    if doSmoothing == 1
        ...
        for jj=1:nFreqs
            ...
            ppr = csaps(t,mr,smoothing); % piecewise polynomial real coefficients
            ppi = csaps(t,mi,smoothing); % piecewise polynomial imaginary coefficients
            mr_sm = ppval(ppr,t);
            mi_sm = ppval(ppi,t);
            % plot(mr)
            % hold on
...
%     ii=80;
%     subplot(2,1,1)
%     Chunk = Clicks(indx(ii):indx(ii)+nSamples,:); 
%     cutN = 1;
%     h = hann(cutN*2);
%     h = h(1:cutN);
%     H = repmat(h,1,size(Chunk,2));
%     Chunk(1:cutN,:) = H .* Chunk(1:cutN,:);
%     Chunk = Chunk(1:100,:);
%     Chunk = flipud(Chunk);
%     Chunk(1:cutN,:) = H .* Chunk(1:cutN,:);
%     Chunk = flipud(Chunk);
%    
%     chunk80 = mean(Chunk,2);
%     plot(Chunk,'r')
%     hold on
%     subplot(2,1,2)
%     plot(20*log10(abs(fft(mean(Chunk,2),fs))),'r')
%     hold on
%     xlim([0 20000])
%     ii=1;
%     subplot(2,1,1)
...
%     hold on
%     subplot(2,1,2)
%     plot(20*log10(abs(fft(mean(Chunk,2),fs))),'b')
%     hold on
%     xlim([0 20000])
% 
%     figure
%     plot(chunk80-chunk1,'g')
