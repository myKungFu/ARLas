function [MEMR] = analyzeMEMR_v10(pathName,fileNameL,fileNameR,subjectName,runNumber)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    MEMR = runme(Clicks,nSamples,nClicks,indx,time,fs,peakFreqs,noiseLvl,noiseRate,...
        subjectName,runNumber,elicitor,fileNameL,fileNameR,pathName,d.name,doSmoothing,doAR);

end
% INTERNAL FUNCTIONS ------------------------------------------------------
function [MEMR] = runme(Clicks,nSamples,nClicks,indx,time,fs,peakFreqs,noiseLvl,noiseRate,...
    subjectName,runName,elicitor,fileNameL,fileNameR,pathName,dname,doSmoothing,doAR)
    ...
    for ii=1:nClicks % loop across each click position
        disp([num2str(ii),' of ',num2str(nClicks)])
        Chunk = Clicks(indx(ii):indx(ii)+nSamples,:); % contains all the sweeps

        if doAR == 1
            % artifact rejection across 15 sweeps
            RMS = sqrt(mean(Chunk(40:120,:),1).^2);
            tolerance = 6;
            doPlot = 0;
            [rejectIndx,nRejects] = newAR2(RMS,tolerance,doPlot);
            if ~isempty(rejectIndx)
                doPlot = 0;
                [rejectIndx,nRejects] = newAR2(RMS,tolerance,doPlot);
                %Chunk = AR_engine(Chunk,rejectIndx);
                Chunk(:,rejectIndx) = 0;
            else
            end
        end
        ...
        nfft = size(Chunk,1);
        FT = fft(Chunk,nfft); % contains all sweeps at all frequencies for a given click position
        fIndx1 = 1;
        fIndx2 = size(Chunk,1)/2;

        FT = FT(fIndx1:fIndx2,:); % cut down to size (non-aliased part only)
    
        % there is slow drift across sweeps (not related to the MEMR?) -----------------------------------------
        % Find the drift by fitting real and imgaginary parts with a smoothing spline.
        if doSmoothing == 1
            nFreqs = size(FT,1);
            for jj=1:nFreqs
                if mod(jj,1000)==0
                    disp(['  ',num2str(jj),' of ',num2str(nFreqs)])
                end
                %m = M(:,:,jj);
                %[rows,cols] = size(m);
                m = FT(jj,:);
                ...
                mi = imag(m(:));
                t = (1:1:length(mr))';
                smoothing = 0.01; %0.01; % smoothing factor is a good sm factor
                ...
                ppr = csaps(t,mr,smoothing,[],w); % piecewise polynomial real coefficients
                ppi = csaps(t,mi,smoothing,[],w); % piecewise polynomial imaginary coefficients
                mr_sm = ppval(ppr,t);
                mi_sm = ppval(ppi,t);
                % plot(mr)
                % hold on
                % plot(mr_sm)
                %pause
                correction = (mr_sm+1i*mi_sm).';
                offset = mean(mr_sm+1i*mi_sm);
                mcorr = (m ./ correction) + (offset-1); % corrected m
        ...
        % cut back down to analysis size
        % take the fft 
        % save in a matrix
        winN = 15; % number of samples to reduce (before reflection comes back)
        clickFixed = clickFixed(1:100+winN,:); % We defined the cutoff of our Signal over here !
    
    
        h = hann(winN*2);
        h = h(1:winN);
        H = repmat(h,1,size(clickFixed,2));
        clickFixed(1:winN,:) = clickFixed(1:winN,:) .* H;
        cutSample = 150;
        clickFixed(cutSample:end,:) = [];
        clickFixed = flipud(clickFixed);
        clickFixed(1:winN,:) = clickFixed(1:winN,:) .* H;
        clickFixed = flipud(clickFixed);
        
        binWidth = 100; % bw = fs / nfft2
        nfft2 = ceil(fs / binWidth);
        FT = fft(clickFixed,nfft2);
        frequency = (0:1:nfft2-1)'*(fs/nfft2);
        [~,indx1] = min(abs(frequency - 125));
        %[~,indx2] = min(abs(frequency - 10000));
        FT = FT(indx1:indx1+(nfft/2)-1,:).';
        frequency = frequency(indx1:indx1+(nfft/2)-1);
        FT = FT / (nfft/2); % scale the magnitude
        M(ii,:,:) = FT;
        %mcorr = [mcorr,zeros(1,nRejects)];
...
%     %frequency = frequency(1:nfft/2);
%     figure
%     plot(frequency,20*log10(abs(q1)/.00002),'b')
%     hold on
%     plot(frequency,20*log10(abs(q80)/.00002),'r')
%     
%     figure(2) % dB change
%     Q = abs(q80)./abs(q1);
%     K = size(Q,2);
%     Qbar = mean(Q,2);
%     SEM = (std(Q.')./sqrt(K))';
%     plot(20*log10(Q),'Color',[.7,.7,.7])
%     hold on
%     plot(20*log10(Qbar),'b','LineWidth',2)
%     plot(20*log10(SEM),'k','LineWidth',2)
%     
%     figure(110)
%     plot(20*log10(Qbar ./ SEM))
%     title('SNR')
%     hold on

% -------------------------------------------------------------------------
    t = (1:1:160)'; % time of click in steps of 1
    nIterations = 200; % number of bootstrap iterations
    mr_sm = zeros(160,nIterations);
    mi_sm = zeros(160,nIterations);
    for jj=1:nFreqs
        m = M(:,:,jj); % size m = 160 x 15
        w = W(:,:,jj);
        Mr = real(m);
        Mi = imag(m);
        ww = sum(w,2);
        for kk=1:nIterations
            indxkk = ceil(rand(15,1)*15);
            % take the weighted mean
            mr = sum(Mr(:,indxkk),2)./ ww;
            mi = sum(Mi(:,indxkk),2)./ ww;
            
            sm = 0.00001;
            ppr = csaps(t,mr,sm,[],ww); % piecewise polynomial real coefficients
            ppi = csaps(t,mi,sm,[],ww); % piecewise polynomial imaginary coefficients
            mr_sm(:,kk) = ppval(ppr,t);
            mi_sm(:,kk) = ppval(ppi,t);
        end

        
        for mm=1:size(mr_sm,1)
            [post] = calculatePosterior_v2(mr_sm(mm,:)');
            hdi1R(mm,1) = post.hdi95(1);
            hdi2R(mm,1) = post.hdi95(2);
            hdi3R(mm,1) = post.hdi68(1); % 68% hdi, corresponds to 1 sd, normally distributed
            hdi4R(mm,1) = post.hdi68(2);
            mymodeR(mm,1) = post.mod;
        end
        for mm=1:size(mi_sm,1)
            [post] = calculatePosterior_v2(mi_sm(mm,:)');
            hdi1I(mm,1) = post.hdi95(1);
            hdi2I(mm,1) = post.hdi95(2);
            hdi3I(mm,1) = post.hdi68(1); % 68% hdi, corresponds to 1 sd, normally distributed
            hdi4I(mm,1) = post.hdi68(2);
            mymodeI(mm,1) = post.mod;
        end
