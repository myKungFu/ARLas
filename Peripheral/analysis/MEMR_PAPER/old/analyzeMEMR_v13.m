function [MEMR] = analyzeMEMR_v13(pathName,fileNameL,fileNameR,subjectName,runNumber)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [MEMR] = analyzeMEMR_v13(pathName,fileNameL,fileNameR,subjectName,runName)
%
...
% Last Updated: October 19, 2023 -- ssg -- version 13. Still to do: Analyze
%               and Figures for both MEMR and Inciden   t part seperatley.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [analysisPart1] = runme(Clicks,nSamples,nClicks,indx,time,fs,peakFreqs,noiseLvl,noiseRate,...
    ...
    if stabilityCheck == 1
        ...
        for ii=1:nClicks % loop across each click position (1-160)
            ...
            % % % q = wf.fpl;
            % % % q2 = wf.rpl;
            % % % q = repmat(q,1,size(Clicks,2));
            % % % q2 = repmat(q2,1,size(Clicks,2));
            % % % 
            q(1:winN,:) = q(1:winN,:) .* H;
            q = flipud(q);
            q(1:winN,:) = q(1:winN,:) .* H;
...
function [] = plot1(analysisPart1_inc,analysisPart1_mem)
    ...
    if ja == 1
        ...
        textPosition1 = [5, 12];  % Position for MEMR Wave legend
        textPosition2 = [5, 10];  % Position for Incident Wave legend
        textColor1 = '#EDB120';  % Orange color
        textColor2 = '#4DBEEE';  % Blue color
        text(textPosition1(1), textPosition1(2), 'MEMR Wave', 'Color', textColor1);
        text(textPosition2(1), textPosition2(2), 'Incident Wave', 'Color', textColor2);

        ylim([-5, 5]); 
        plot(20*log10(abs(z)),'Color','#EDB120')
        xlabel('Time (samples)')
        ...
        plot(20*log10(trend),'Color','#EDB120')
        xlabel('Time (samples)')
        ylabel('Trend (dB)')
        ...
        hold on
        plot(z,'Color','#EDB120')
        plot1(analysisPart1_mem,analysisPart1_mem)
    ...
    if ja == 2
        ...
        subplot(2,1,1)
        ylim([-5, 5]); 
        plot(20*log10(abs(z)),'Color','#4DBEEE')
        xlabel('Time (samples)')
        ylabel('Mag Change (dB)')
        ...
        hold on
        subplot(2,1,2)
        plot(20*log10(trend),'Color','#4DBEEE')
        xlabel('Time (samples)')
        ylabel('Trend (dB)')
