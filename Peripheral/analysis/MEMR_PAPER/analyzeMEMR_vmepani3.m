function [ C_mem, C_inc] = analyzeMEMR_vmepani3(Clicks,counter,stabilityCheck)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [analysisPart1_inc,analysisPart1_mem] = analyzeMEMR_vmepani3(pathName,subjectName,runNumber)
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % ---------------------------------------------------------------------
    doJitterFix = 0; % set this to zero. All jitter was removed previously
    % ---------------------------------------------------------------------

    % extract the raw recordings -----

    [xx,yy,zz] = size(Clicks);
    Q = Clicks(:);
    Q = reshape(Q,xx,yy*zz); % reshape clicks into 2-D matrix
    if doJitterFix == 1
        Clicks = unjitter(Q,'none');
    else
        Clicks = reshape(Clicks,999,288);
    end

    %Clicks = ARLas_hpFilter(Clicks,fs,100); remove this; done previously
    ...
    % C = median(C,2);
    RMS = sqrt(mean((Clicks.^2),1)); % elicitor RMS
    ...
    pSPL = 20*log10(max(abs(Clicks),[],1)/.00002); % level of the click in pSPL
    ...
    % analyze MEMR --------------------------------------------------------
    stabilityCheck = 0;
    C_mem = runme(Clicks,stabilityCheck,counter);
    stabilityCheck = 1;
    C_inc = runme(Clicks,stabilityCheck,counter);
...
% INTERNAL FUNCTIONS ------------------------------------------------------
function [MEMR] = runme(Clicks,stabilityCheck,counter)
   ...
   indx = 482;
    ...
    if stabilityCheck == 1 % this is for incident part of the click -----
        ...
        H = repmat(h,1,size(Clicks,2));
        q = Clicks(indx-winN:indx+winN,:); % reflected part of the sweep starting 14 samples after incident click
        q = q .* H;
        [rows,cols] = size(q);
        C = reshape(q,29,12,cols/12); % matrix of clicks: 114 samples, 12 reps, 24 elicitor levels       
        ...
        H = repmat(h,1,size(Clicks,2));
        q = Clicks(indx:indx+clickN-1,:); % reflected part of the sweep starting 14 samples after incident click
        q(1:winN,:) = q(1:winN,:) .* H;
        q = flipud(q);
        q(1:winN,:) = q(1:winN,:) .* H;
        q = flipud(q);
        [rows,cols] = size(q);
        C = reshape(q,114,12,cols/12); % matrix of clicks: 114 samples, 12 reps, 24 elicitor levels
    ...
    if stabilityCheck ~=5 % this is hack to make this always happen
       ...
       for jj=1:nFreqs
        ...
        for jj=1:nFreqs % loop across frequencies ---
            ...
            sumw = sumw + eps; % avoid division by zero
...
%           mi = sum(Mi(:,:).*wi,1)./ sumwi;
            ...
            dy = finish - start;
            slope = dy/dx;
            lineI = ((0:1:n-1)*slope)+start;
            mi = mi - lineI;

            mrrr = [mr,mr,mr];
            miii = [mi,mi,mi];
            www = [sumw,sumw,sumw];
            sm = 0.1; %0.0001; % was 0.00001;  smoothing factor (smaller numbers are more smooth)
            ...
            baseline = mean([z_sm(1),z_sm(end)]);
            z_sm = z_sm ./ baseline;
            d = z_sm; % normalize z_sm
...
function [Clicks] = unjitter(Clicks,subjectName)
    Q = Clicks;
    Q = Q(1:110,:); % isolate the first click (of two in each column)
    ...
    if ~isempty(badIndx)
        ...
        Clicks = D2; %reshape(D2,size(D2,1)*2,size(D2,2)/2);
    end
end
function [CFT,TREND,W] = memrDetrend(CFT)
    ...
    for ii=1:nFreqs % do this individually for each frequency
        ...
        smoothing = 0.0000001; % smoothing factor--0 is a straight line, 1 is cubic spline
        xxx = (1:1:length(mr)*3)';
        xxx = xxx ./ xxx(end);
