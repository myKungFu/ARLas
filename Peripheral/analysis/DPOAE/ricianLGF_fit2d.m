function [jar,h1,h2] = ricianLGF_fit2d(Z,Sz,X,nSweeps)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [jar,h1,h2,h3,h4,h5] = ricianLGF_fit2d(Z,Sz,X);
% This is a spin-off of version _fit2c. This one is used for data in the
% lab as of May 21, 2025 -- ssg
%
...
function [jar] = getCIs(jar,nIterations,B)
    ...
    alfa = 0.05;
    ciFit(:,1) = quantile(real(YHAT),alfa/2,2);
    ciFit(:,2) = quantile(real(YHAT),1-(alfa/2),2);
    ...
    ciSlope(:,1) = quantile(real(SLOPE),alfa/2,2);
    ciSlope(:,2) = quantile(real(SLOPE),1-(alfa/2),2);
    CISlope = 20*log10(ciSlope);
    
    ciD2(1,1) = quantile(real(D2YDX2),alfa/2);
    ciD2(2,1) = quantile(real(D2YDX2),1-(alfa/2));

    ciMLE(1,1) = quantile(real(MLE(:,1)),alfa/2);
    ciMLE(1,2) = quantile(real(MLE(:,1)),1-(alfa/2));
    ciMLE(2,1) = quantile(real(MLE(:,2)),alfa/2);
    ciMLE(2,2) = quantile(real(MLE(:,2)),1-(alfa/2));
