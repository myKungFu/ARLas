function [MEMR_inc,MEMR_mem,h1] = analyzeMEMR_v16(header,Clicks,headerN,Noise,subjectName,runNumber)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [analysisPart1_inc,analysisPart1_mem] = analyzeMEMR_v16(pathName,subjectName,runNumber)
%
% Analyze MEMR data, new goodman test.
% Call this function using goodmanMEMR_analysis_v16.m
%
...
% Last Updated: January 31, 2025 -- ssg Changed the way detrending was
% smoothing the results across long duration of sweeps Updated to version
% 16
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% this is for the ARO talk, fig 3 ----
    ...
    longTimei = (0:1:length(q2i(:))-1)'*0.05;
    shortTime = (0:1:size(q2,1)-1)'*0.05;
    shortTimei = (0:1:size(q2i,1)-1)'*0.05;
   ...
   figure(3)
   subplot(2,1,1)
    plot(longTimei,qi(:),'Color',[.7 .7 .7])
    hold on
    plot(longTimei,ttti(:),'b','LineWidth',2)
    xlim([0,120])
    % ylim([ymin,ymax])
    xlabel('Time (s)','FontSize',14)
    hy1 = ylabel('Real','FontSize',14);
    ...
    box on
  subplot(2,1,2)
    plot(longTimei,q2i(:),'Color',[.5 .5 .5])
    xlim([0,120])
    % ylim([ymin,ymax])
    xlabel('Time (s)','FontSize',14)
    hy1 = ylabel('Real','FontSize',14);
  ...
  % ------ % show smoothing
   ...
   subplot(2,1,1)
    ...
    hold on    
    plot(shortTime,real(z_sm),'Color',[0 0 1],'LineWidth',2)
    ...
    box on
  subplot(2,1,2)
    plot(shortTime,real(d),'Color',[0 0 1],'LineWidth',2)
    %plot(shortTime,mean(q,2),'r')
    xlim([0,8])
    % ylim([0.9 1.01])
    xlabel('Time (s)','FontSize',14)
    hy1 = ylabel('Normalized','FontSize',14);
...
% January 31, 2025
% Noticed a problem when re-using the weighting vector created for long
% duration deterending and applying it to short times (single sweeps).
% Here, we find a new weighting matrix based on deviation from the median
% across sweeps.
MR = reshape(mr,rows,cols);     
q = median(MR,2);
Q = repmat(q,1,cols);
D = abs(MR - Q);
w = ones(size(mr(:))); % weighting vector (set to ones)
...
doPlotAR = 0;
multiplier = 1.5;
[rejectIndx,nRejects] = newAR2(D(:)',multiplier,doPlotAR); 
w(rejectIndx) = 0; % downweight noisy samples to zero
w = reshape(w,rows,cols);
W(ii,:,:) = w.';
