classdef arlas_audiometer < handle
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
properties (SetAccess = public) 
    ...
    simulatePatient % use a patient simulator (as opposed to a real patient)
    simPatientThd % set the simulated patient threshold (a fixed value for all freuqencies)
    
end
methods
    function obj = arlas_audiometer(arlasObj,freqs) % initialize object of class initARLas
        ...
        obj.getLpf;
        obj.simulatePatient = 0; % default to NOT simulating patient   
        obj.simPatientThd = 35; % default simulated patient threshold
        if nargin == 2
            freqs = sort(freqs(:));
        else
            freqs = [0.5 1 2 4 8 9 10 11.2 12.5 14 16]' * 1000; % default frequency set
        end
        obj.freqs = freqs(:)';
        ...
        obj.lvlRefs = [{'FPL'},{'SPL'},{'IPL'},{'None'}]; % initialize to fpl (forward pressure level)
        obj.stepSizes = [5,2]; % level step size options
        obj.minOutput = zeros(1,obj.nFreqs);  % initialize to zeros
        obj.maxOutput = ones(1,obj.nFreqs) * 100;  % initialize to 100 dB FPL
        ...
        obj.autorunNow = 0; % initilize to manual control
        obj.earNow = 1;
    ...
    function initGui(varargin) % initialize the arlas graphical user interface
       ...
       obj.H.Position = [809.8000   49.8000  544.8000  784.8000]; %[887.0000   67.4000  544.8000  744.4000];
            ...
            obj.REF = uicontrol('Parent',obj.CONTROL,'Style','popup',...
                'String',[{'FPL','SPL','IPL','None'}],'FontSize',12,'BackgroundColor',...
                'white','HorizontalAlignment','Left','Units','Normalized',...
            ...
            obj.h_switchPlots = uicontrol('Parent',obj.VIEW,'Style','togglebutton',...
                ...
                'TooltipString','Switch from Progress view to Audiogram',...
                'String','Switch View','BusyAction','queue','Interruptible','on');
            % -----            
    ...
    function buttonManager(varargin) % change colors and states of buttons on bottom bar
        ...
        try
            ...
            % 10s are for arlas_audiometer gui
    ...
    function presentManager(varargin) % control which input channel is currenly being viewed
        obj = varargin{1};
                    ...
                    while any(obj.doneL == 0) % while left ear is not finished ----
                        for ii=1:obj.nFreqs
                            ...
                            while obj.doneL(ii) == 0
                                obj.freqUpManager
                                obj.prepStim
                                if obj.REF.Value == 4 % if calibration is 'None', skip the insitu part
                                    stim = obj.stimNow;
                                    lvl = obj.lvlNow;
                                    maxx = max(obj.levels);
                                    ratio = lvl / maxx;
    ...
    function refManager(varargin) % control level reference
        ...
        elseif obj.REF.Value == 4
            obj.refNow = obj.lvlRefs(4);    
        end
    end
    function autoManager(varargin) % control which input channel is currenly being viewed
        obj = varargin{1};
        if obj.AUTO.Value == 1
            obj.autorunNow = 0;
            obj.AUTO.BackgroundColor = [1 1 1];
    ...
    function doCal(varargin)
        ...
        % check to see if calibration is wanted.
        value = obj.REF.Value;
        if strcmp(obj.REF.String(value),'None')
            obj.iscS1L = [];
            obj.iscS2L = [];
            obj.iscS1R = [];
            obj.iscS2R = [];
    ...
    function doSave(varargin)
        ...
        saveFileName = [ao.subjectID,'_analyzedAUDIO.xls'];
        saveFileName = ARLas_saveName(savePath,saveFileName);
        obj.writeData2Excel(savePath,saveFileName,freqs,ThdL,ThdR)
        
        obj.buttonManager(20)
    end
    function doSwitch(varargin)
        obj = varargin{1};
        ...
        if obj.h_switchPlots.Value == 0
            ...
            axes(obj.h_track)
            axis off
            %obj.h_pif.Visible = 'off';
            %obj.h_track.Visible = 'off';
            try delete(obj.marker1)
            catch
            end
            try delete(obj.marker2)
    ...
    function getStim(varargin) % control which input channel is currenly being viewed
        % create stimulus for audiometer
        % stimulus length is 500 ms
        % response window is 2 seconds
        % onset/offset ramps are 5 ms
        obj = varargin{1};
        
        len = 0.5; % stimulus length (s)
        ...
        winLen = 0.005;
        winN = round(winLen * obj.fs);
        h = hann(winN*2);
        h = h(1:winN);
        H = repmat(h,1,obj.nFreqs);

        for ii=1:obj.nFreqs
           Stim(:,ii) = sin(2*pi*obj.freqs(ii)*t);  
        end

        Stim(1:winN,:) = Stim(1:winN,:) .* H;
        Stim = flipud(Stim);
        Stim(1:winN,:) = Stim(1:winN,:) .* H;
        Stim = flipud(Stim);
        ...
        extraCount = 2; % number of extra seconds
        zpad = zeros(foldN*extraCount,1); % this zero padding is to make everyting fold evenly
        qq = 1;
        stim = obj.Stimuli(:,qq);
    ...
    function playStim(varargin) % present the stimulus to subject
        ...
        if obj.AUTO.Value == 1 % if in manual mode, 1 ms pause
            pauseLen = 0.001;
        else % if in auto mode, give some randomness to the presentation rate
            pauseLen = rand(1,1)*1;
        end
    ...
    function getResponse(varargin) % extract the recordings to see if button was pressed (tone was heard)
        ...
        if obj.simulatePatient == 0
            ...
            if obj.lvlNow > obj.simPatientThd
                r = 1;
            else
                r = 0;
            end
    ...
    function analyze(varargin) % analyze and get next level (if auto)
        obj = varargin{1};
    ...
    function countReversals(varargin)
        obj = varargin{1};
    ...
    function audiometer_plotPI(varargin) % plot audiometer PI function
        ...
        if isempty(LVL)
            hold off
            plot(0,0,'.w') % plot individual responses
            hold on
            
        else
            hold off
            plot(LVL,RESP+0.01*randn(size(LVL)),'.b') % plot individual responses
            hold on
            plot(u,v,'ob') % plot averaged response rates
            if N > 6
                plot(cix,ci(:,1),'g--')
                plot(cix,ci(:,2),'g--')
                line([thd-(piw(end)/2) thd+(piw(end)/2)],[0.5 0.5],'Color',[0 1 0],'LineWidth',2,'LineStyle','-')
            end
            plot(cix,pif,'r') % plot logistic fit
            try
            plot(thd,0.5,'or') % plot current threshold
            catch ME
                return
            end
            for kk=1:length(u) % show how many presentations at each level
                text(u(kk),-0.05,num2str(nn(kk)), 'Rotation', 0, 'VerticalAlignment', 'middle','FontSize',8)            
            end
        ...
        if isempty(LVL)
            ...
            indx0 = find(RESP == 0);
            hold off
            plot(x(indx1),LVL(indx1),'go','LineWidth',2)
            hold on
            plot(x(indx0),LVL(indx0),'rx','LineWidth',2)
            plot(x,LVL,'.-b')
        end
        xlabel('Trial Number')
        ylabel('Stim Level')
        ...
        obj.marker2 = plot(N+1,obj.lvlNow,'k^','MarkerFaceColor',[0 0 0]);
        if N > 6
            line([1 N+1],[thd thd],'Color',[0 0 0],'LineWidth',0.5,'LineStyle','--')
        end
        grid on
    end
    function audiometer_plotAG(varargin) % plot audiometer Audiogram
        ...
        for ii=1:nf
            axes(h1)
            line([ff(ii),ff(ii)],[ymin,ymax],'Color',[0 0 0],'LineWidth',0.5,'LineStyle','-') 
            hold on
        end
        ylim([ymin,ymax])
        xlim([xmin,xmax])
        for ii=1:length(LL)
            axes(h1)
            line([xmin,xmax],[LL(ii),LL(ii)],'Color',[0 0 0],'LineWidth',0.5,'LineStyle','-') 
        end
        ...
        if all(isnan(thdL)) && all(isnan(thdR)) % there is nothing else to plot
            ...
            for ii=1:obj.nFreqs
                plot(freqs(ii),thdL(ii),'bx','MarkerSize',12,'LineWidth',1)
            end
            for ii=1:obj.nFreqs
                plot(freqs(ii),thdR(ii),'ro','MarkerSize',11,'LineWidth',1)
            end
            plot(freqs(:)',thdL,'b:','LineWidth',0.5)
            plot(freqs,thdR,'r:','LineWidth',0.5)
    ...
    function checkIsFinished(varargin)
        obj = varargin{1};
    ...
    function audiometerProbes(varargin)
        try
            obj = varargin{1};
            obj.probeInputL = varargin{2};
            obj.probeOutputL = varargin{3};
            obj.probeInputR = varargin{4};
            obj.probeOutputR = varargin{5};
            obj.clickerInput = varargin{6};
        catch ME
            error('Unable to process inputs for function audiometerProbes')
        end
    end
    ...
    function writeData2Excel(varargin)
        ...
        % write LEFT ear -------------------------------
            range = 'A3';
            xlswrite([pathName,fileName],{'Freq (Hz)'},sheet,range);
            range = 'A4';
            xlswrite([pathName,fileName],freqs(:,1),sheet,range);

            range = 'B3';
            xlswrite([pathName,fileName],{'Thd L'},sheet,range);
            range = 'B4';
            xlswrite([pathName,fileName],ThdL,sheet,range);

            range = 'C3';
            xlswrite([pathName,fileName],{'Thd R'},sheet,range);
            range = 'C4';
            xlswrite([pathName,fileName],ThdR,sheet,range);
