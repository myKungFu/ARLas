classdef thevCal_new2 < handle
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Class definition thevCal_new2
...
properties (SetAccess = public)
    ...
    cut1
    cut2
    stepCounter % using iterative method from Nørgaard & Neely, keeps track of which of the 5 steps you are on
end

methods
    function t = thevCal_new2(recordingParams,stimulus,recordings)
    ...
    function [] = calculate(obj)
        ...
        calculatePL(obj);                   % I.  CALCULATE PL (load pressure; pressure measured in the tubes)
        ...
        % Iterative method from Nørgaard & Neely, 2018
        %
        % 1. Let ???=0 and find the set of ??? and ?? (for n=1…N) that minimize ??? for 2<?<8 kHz.
        %    zew = ??? = evanescent-wave impedance
        %    zfl = ??? = flow loss impedance
        %    cavityLengths_optimal = ?? = cavity lengths (we also have cavityLengths_nominal)
        %    epsilon = ??? = waveguide pressure estimation error
        %
        %    We speculate that evanescent waves play a small role below 8 kHz, hence 
        
        obj.stepCounter = 1; % first step in iterative fit
        obj.optSwitch = 'L';
        [obj,fval,exitFlag,output] = fminsearchOBJ(@findOptimalParams,obj,op);
        
        % 2. Estimate ??? from ??. If ???<0, then let ?1=????, otherwise let ?1=0.
        % tau = ??? = parallel-compliance delay
        obj.stepCounter = 2;
        calculateTau(obj)
        
        % 3. Let ???=??0??1 and find the set of ??? and ?? (for n=1…N) that minimize ??? for 2<?<8 kHz.
        obj.stepCounter = 3;
        obj.optSwitch = 'L';
        [obj,fval,exitFlag,output] = fminsearchOBJ(@findOptimalParams,obj,op);
        
        
        % 4. Let ???=??0??(???)?5?=1 and find the set of ?? (for m=1…5) that minimize ??? for 2<?<18 kHz.
        obj.stepCounter = 4;
        obj.optSwitch = 'T';
        op=optimset('Display','off','MaxIter',100);
        [obj,fval,exitFlag,output] = fminsearchOBJ(@findOptimalParams,obj,op);
        
        % 5. Let ???=??0??(???)?5?=1 and find the set of ??? and ?? (for n=1…N) that minimize ??? for 2<?<8 kHz.        
        obj.stepCounter = 5;
        obj.optSwitch = 'L';
        [obj,fval,exitFlag,output] = fminsearchOBJ(@findOptimalParams,obj,op);
...
%         
    ...
    function [] = calculateTau(obj) % calculate parallel compliance delay
        % ???=?????0
        % ?????12???{1??}.
        % Estimate ??? from ??. If ???<0, then let ?1=????, otherwise let ?1=0.
        cpar = 1./(2*pi*obj.freq).*(imag(1./obj.ZS)); % imaginary part of ZS (estimates parallel compliance)
        obj.tau = cpar * obj.z0;
        indx = obj.tau < 0;
        obj.tau(indx) = -obj.tau(indx);
        obj.tau(~indx) = 0;
    end
    function [] = calculatezew(obj) % 2b) CALCULATE zew (evanescent wave impedance of tubes)
        if obj.stepCounter == 1
            obj.zew = zeros(obj.fmaxIndx - obj.fminIndx+1,1);
        elseif obj.stepCounter == 2
            
        elseif obj.stepCounter == 3
            obj.zew = 1i * obj.z0 .* obj.omega .* obj.tau;
        elseif obj.stepCounter == 4
            M = 5; % not sure why???
            obj.tau = zeros(1,M); % wavespread estimate (from parallel compliance delay)
            ...
            obj.tau = repmat(obj.tau,1,M); % row vector
...
%         obj.zew = phi * obj.z0;
    ...
    function [] = calculateError(obj) % CALCULATE ERROR (compare calculated and measured pressure)
        ...
        [~,iMin] = min(abs(fmin-obj.freq));
        [~,iMax] = min(abs(fmax-obj.freq));
        ...
        accumulatedPressure = accumulatedPressure(iMin:iMax);
            for ii=1:obj.nCavities % loop across cavities
               pressureDiff = obj.PL(iMin:iMax,ii) - obj.PS(iMin:iMax,1) .* obj.ZLi(iMin:iMax,ii) ./ (obj.ZS(iMin:iMax,1) + obj.ZLi(iMin:iMax,ii)); % Numerator |Pc - Ps * Zc / (Zs + Zc)|
               ...
               s2 = s2 + sum(abs(obj.PL(iMin:iMax,ii)).^2); % sum of squares: square the measured cavity pressure and sum across frequency
            end
...
function [epsilon] = findOptimalParams(obj)
