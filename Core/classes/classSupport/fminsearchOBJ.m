function [obj,fval,exitflag,output] = fminsearchOBJ(funfcn,obj,options,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [obj,fval,exitflag,output] = fminsearchOBJ(funfcn,obj,options,varargin)
%
%
...
% Change the code here so that input can be an object. But assume this is
% being used for thev cal, so that optimization is for tube lengths.

% Check for non-double inputs
% removed by SSG 6/27/2018
% if ~isa(x,'double')
%   error('MATLAB:fminsearch:NonDoubleInput',...
%     getString(message('MATLAB:optimfun:fminsearch:NonDoubleInput')));
% end
if strcmp(obj.optSwitch,'L') % length
    x = obj.cavityLengths_optimal(:);
elseif strcmp(obj.optSwitch,'T') % tau
    x = obj.tau(:);
elseif strcmp(obj.optSwitch,'B') % both simultaneously
    x1 = obj.cavityLengths_optimal(:);
    x2 = obj.tau(:);
    x = [x1;x2];
end
...
function [obj] = switchMe(obj,x)
    if strcmp(obj.optSwitch,'L')
        obj.cavityLengths_optimal = x;
